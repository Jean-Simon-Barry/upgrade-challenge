    ...
    WITH RECURSIVE calendar AS (
        ...
        SELECT
            CASE camping_reservation.reservation_dates && calendar.left
                WHEN TRUE THEN daterange(lower(calendar.left), lower(camping_reservation.reservation_dates * calendar.left))
                ELSE daterange(lower(calendar.right), lower(camping_reservation.reservation_dates * calendar.right))
            END AS left,
            CASE camping_reservation.reservation_dates && calendar.left
            ...
            CASE camping_reservation.reservation_dates && calendar.right
                WHEN TRUE THEN daterange(upper(camping_reservation.reservation_dates * calendar.right), upper(calendar.right))
                ELSE daterange(upper(camping_reservation.reservation_dates * calendar.left), upper(calendar.left))
            END AS right
        FROM calendar
